...
async function getAppModuleContent(tree: UnitTestTree): Promise<string> {
  const workspace = await getWorkspace(tree);
  const project = getProjectFromWorkspace(workspace, 'nebular');
  const appModulePath = getAppModulePath(tree, getProjectMainFile(project));
  return tree.readContent(appModulePath);
}
...
describe('ng-add', () => {
  ...
  function runNgAddSchematic(options: Partial<NgAddOptions> = {}) {
    return runner.runSchematic('ng-add', { project: 'nebular', ...options }, appTree);
  }

  function runSetupSchematic(options: Partial<NgAddOptions> = {}) {
    return runner.runSchematic('setup', { project: 'nebular', ...options }, appTree);
  }

  function runPostInstallSchematic(options: Partial<NgAddOptions> = {}) {
    return runner.runSchematic('post-install', { project: 'nebular', ...options }, appTree);
  }
  ...
  describe('ng-add', () => {
    ...
    it('should add @nebular/eva-icons in package.json', async () => {
  ...
  it('should register NbThemeModule.forRoot()', async () => {
    ...
    const appModuleContent = await getAppModuleContent(tree);
  ...
  it('should not add NoopAnimationsModule if BrowserAnimationsModule is set up', async () => {
    ...
    expect(fileContent.getFullText()).not.toContain(
      'NoopAnimationsModule',
